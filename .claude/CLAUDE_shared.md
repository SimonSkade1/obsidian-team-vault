...
5. Every file in `projects-tasks-notes/` carries these seven properties, in this order: `status`, `priority`, `parent`, `owner`, `next_action_by`, `not_before`, `subscribers`.
	...
	2. `priority`: 1–10, 10 = most important; empty counts as 5; views sort a project by the highest priority among the items below it that fall in the viewer's sections 1–4, when that is higher. A ranking for humans, never an execution order.
