...
`effective_priority` (the row sort of every view) = max(own `priority`, or 5 if empty; the highest `priority` set on any non-terminal descendant — empty ones below contribute nothing). Formulas can't recurse or query the vault, so each row walks its whole tree once: `anc1`–`anc4` → `top` (the tree's root project, or the row itself) → `lvl1`–`lvl4` (the active nodes by depth below `top`, each level = the `backlinks` of the previous level whose `parent` is in it) → `tree` (the nodes that have a priority, each as the string `"<priority>|<ancestor path>|…|"`, since a formula can't carry a pair) → a node's effective priority filters `tree` by `"|<its path>|"`. The same filter with `anc1`–`anc4` gives `seg1`–`seg4`. Limit: four levels below `top`, like `anc4`. Cost: one tree walk per row, fine at hundreds of files.
