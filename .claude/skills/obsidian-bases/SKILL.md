...
1. `parent == this` in `subtasks.base` fills `parent` with the project the table is embedded in. It sits in a global `or` with `formula.anc2 == this` … `formula.anc4 == this` (the deeper descendants for the tree views); those rules are harmless because only rules whose left side is a note or file property are written. The current view's filters count as well as the global ones (`Mine` / `All` repeat `parent == this` to stay direct-children-only).
